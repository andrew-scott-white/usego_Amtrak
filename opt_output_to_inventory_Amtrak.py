...
override_phi = False # set to true if you don't want to scale output to Amtrak's fraction of grid load
#run_name = 'base'
run_name = 'amtk_current' #insert relevant run name (eg: no_nuclear, base, etc..., to name the read in files and the save out files)
...
if run_name == 'amtk_current':
    ...
    print(len(hours),'hours in a year (adds the last one in a subsequent line)')
    print(phi_region,'regions with Amtrak grid load')
...
print('NO annual emissions:',emis_tot_NO2, 'Mg/yr')
...
print('NO annual emissions:',emis_tot_SO2, 'Mg/yr')
...
f_out = f'./annual_emissions/inventory_power_plants_scaled_{run_name}.nc'
print('nc file made')
...
nc_out.Contact = 'aswhite@mit.edu'
